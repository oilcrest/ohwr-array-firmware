...
*that 'ptos' points to the highest valid address on the stack.
*
...
*change this by setting the SREG to 0x00 instead. In this case, interrupts would
*be disabled upon task startup. The application code would be responsible for
*enabling interrupts at the beginning of the task code. You will need to modify
*OSTaskIdle() and OSTaskStat() so that they enable interrupts. Failure to do
*this will make your system crash!
*
*              The AVR return stack is placed OS_TASK_STK_SIZE_HARD bytes before
